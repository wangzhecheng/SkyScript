...
class CsvDataset(Dataset):
    def __init__(self, input_filename, transforms, img_key, caption_key, sep="\t", tokenizer=None, root_data_dir=None):
        logging.debug(f'Loading csv data from {input_filename}.')
        df = pd.read_csv(input_filename, sep=sep)
        if root_data_dir is not None:
            df[img_key] = df[img_key].apply(lambda x: os.path.join(root_data_dir, x))
